            ...
            <td colspan="2" align="center">
                ...
                {                   
                    ...
                    switch($arResult["STATUS_ID"]):
                        case "A":
                            if($arResult["PAY_SYSTEM_ID"] == 5):?>
                                <div class="text-align_center" style="margin: 20px 0">
                                    <a href="/personal/order/kvit/?id=<?=$arResult["ID"]?>" target="_blank" style="font-size: 16px">Скачать квитанцию в формате XLS</a>
                                </div>
								...
								<tr>
									<td colspan="2" align="center">
										<span class="tablebodytext">
											Услугу предоставляет сервис онлайн-платежей <b>«Яндекс.Касса»</b>.<br><br>Сумма к оплате по счету:  <?=$arResult['PRICE_FORMATED'];?><br>
										</span>
										<form name="ShopForm" action="https://money.yandex.ru/eshop.xml" method="post">
											...
											<input type="radio" name="paymentType" value="AC" style="display: none" checked="checked" type="hidden"/>

											<br>
											<button type="submit" name="BuyButton" class="bues">Оплатить</button>

											<p>
												<span class="tablebodytext"><b>Обратите внимание:</b> если вы откажетесь от покупки, для возврата денег вам придется обратиться в магазин.</span>
											</p>
										</form>            
									</td>
								</tr>
                        ...
                        case "O":?>
                            К сожалению товара нет на складе
                            <?$srok = CSaleOrderPropsValue::GetList(Array(), Array("ORDER_ID"=>$arResult["ID"], "CODE"=>"SROK"), false, false, Array())->GetNext();
                            if($srok["VALUE"]):?>
                                <br />Ожидаемый срок поставки: <?=$srok["VALUE"]?>
                            <?endif?>
                        <?break;
                        ...
                        case "S":?>
                            Товар поступил на склад, но Вам нужно повторить свой заказ
                        <?break;
