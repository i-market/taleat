...
$arUser = $rsUser->Fetch();
global $name,$lastName,$secondName;
$name = $arUser["NAME"];
                ...
                <td>
                    ...
                    elseif($arProperties["TYPE"] == "TEXT")
                    {
                        switch($arProperties["FIELD_NAME"]){
                            case "ORDER_PROP_1":
                                $temp = explode(" ", $arProperties["VALUE"]);
                                if (count($temp)>1){
                                    $arProperties["VALUE"] = $temp[0];
                                } else {
                                    if (!$arProperties["VALUE"]) $arProperties["VALUE"] = $lastName;
                                }
                            break;
                            case "ORDER_PROP_25":
                                if (!$arProperties["VALUE"]) $arProperties["VALUE"] = $name;
                            break;
                            case "ORDER_PROP_26":
                                if (!$arProperties["VALUE"]) $arProperties["VALUE"] = $secondName;
                            break;
                        }
...
<div style="">
    <table class="sale_order_table props" id="sale_order_props">
        <tr>
            <td colspan="2"></td>
            <td rowspan="9" style="font-size: 18px; padding:0 10px" align="center" valign="middle">
                Работаем только по 100% предоплате!<br /><br />
                Необходимо указать ФИО получателя заказа и адрес доставки.
            </td>
        </tr>
    <?
...
<?/*
<div style="width:350px; display: inline-block; height: 540px; vertical-align: top">
    <table width="100%" height="100%">
        <tr>
            <td style="font-size: 20px" align="center" valign="middle">
                Работаем только по 100% предоплате!<br /><br />
                Необходимо указать ФИО получателя заказа и адрес доставки.
            </td>
        </tr>
