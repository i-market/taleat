<?
	require_once($_SERVER['DOCUMENT_ROOT'].'/bitrix/php_interface/include/myfile.php');
	//////////////////////////
...
if(!empty($PROP)):
	...
	foreach($PROP as $prop_code=>$prop_fields):
		...
		<tr id="tr_PROPERTY_<?echo $prop_fields["ID"];?>"<?if ($prop_fields["PROPERTY_TYPE"]=="F"):?> class="adm-detail-file-row"<?endif?>>
			...
			<?if ($prop_fields["ID"] == 80):?>
				<td width="60%"><?_ShowPropertyFieldCustom('PROP['.$prop_fields["ID"].']', $prop_fields, $prop_fields["VALUE"], (($historyId <= 0) && (!$bVarsFromForm) && ($ID<=0) && (!$bPropertyAjax)), $bVarsFromForm||$bPropertyAjax, 50000, $tabControl->GetFormName(), $bCopy);?></td>
			<?else:?>
