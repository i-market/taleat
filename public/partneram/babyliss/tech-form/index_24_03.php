...
if (!$IS_ERROR && $IS_POST){
    ...
    $book = PHPExcel_IOFactory::load("zakl_new.xls");

    $num = 1;
    $res = CIBlockElement::GetList(Array("ID"=>"DESC"), Array("IBLOCK_ID"=>10, "ACTIVE"=>"Y"), false, Array("nTopCount"=>1), Array("ID", "PROPERTY_NUMER"));
    if($ob = $res->Fetch()){
    ...
    $book->getActiveSheet()->setCellValue('I3', $SC["DATA_ZAKL"]);
    ...
    $book->getActiveSheet()->setCellValue('C13', $IZDEL["NAME"]);
    ...
    $PROP["ITEM_PRODUCTS"] = $_POST["IZDEL"]["NAME"];
...
<?$APPLICATION->IncludeComponent(
    ...
    Array(
        ...
        "FILTER_NAME" => "filter",
        "FIELD_CODE" => Array(""),
        "PROPERTY_CODE" => Array("NUMER", "FORMA", "STATUS", "SC_NAME", "USER_IMGS"),
        "CHECK_DATES" => "N",
        "DETAIL_URL" => "",
        ...
        <tr>
            ...
            <td>
                ...
                <select class="input-form" name="IZDEL[NAME]">
					<?
					/*if ($arProduct = $arProducts->GetNext()):
